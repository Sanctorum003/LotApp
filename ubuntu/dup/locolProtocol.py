...
class ArmProtocol(po.Protocol):
    ...
    def carLeave(self):
        ...
        b=update_user_data(openid,'cur_state',user_state)
        print('cur_state更新成功为',user_state)
        c=update_orders_data(openid,st,'stop_time',stop_Time)
        d=update_orders_data(openid,st,'last_time',sec)
        e=update_orders_data(openid,st,'cost',cost_money)
        
        print('查询得:')
        x = search_from_order_db(openid,st,'openId')
        print('实际:')
        print(openid,st)
